...
| Chat-first config & Web UI settings | In Progress | Typed control-plane config contracts/gateway are implemented; fully supporting declarative `polar.config.json` bootstraps for LLMs, Channels, Automations, and Extensions, all manageable via the Web UI interface without requiring hardcoded script edits |
