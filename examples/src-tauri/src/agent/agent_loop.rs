...
use crate::browser::{BrowserManager, BrowserSettings};
...
impl AgentLoop {
    ...
    pub fn new_with_provider(
        ...
        browser_settings: Option<BrowserSettings>,
        ...
        if let Some(settings) = execution_settings.clone() {
            tool_executor = tool_executor.with_execution_settings(settings);
        }
        if let Some(manager) = browser_manager.clone() {
            tool_executor = tool_executor.with_browser_manager(manager);
        }
        if let Some(settings) = browser_settings.clone() {
            tool_executor = tool_executor.with_browser_settings(settings);
        }
        if let Some(consent) = browser_consent {
            tool_executor = tool_executor.with_browser_consent(consent);
        }
        if let Some(database) = db {
            tool_executor = tool_executor.with_db(database);
        }
        let message_builder =
    ...
    pub async fn run_with_history(
        ...
        loop {
            ...
            if turn > self.config.max_turns {
                let message = format!("Reached maximum turns ({})", self.config.max_turns);
                let _ = event_tx
            ...
            // Parse and emit plan if present in text (legacy)
            if self.config.system_prompt.contains("<plan>") {
            ...
            for tool_use in &tool_uses {
                if tool_use.name == "emit_plan" {
                    if let Some(plan_steps) = self.parse_plan_tool_input(&tool_use.input) {
                        let _ = event_tx.send(AgentEvent::Plan { steps: plan_steps }).await;
                    }
                    tool_results.push(ToolResult::success(
                        tool_use.id.clone(),
    ...
    async fn send_request(
        ...
        match self.provider_config.api_format {
            ...
            ApiFormat::OpenAIResponses => {
                self.send_openai_responses_request(request, event_tx).await
            }
    ...
    /// Send OpenAI Responses format request.
    async fn send_openai_responses_request(
        ...
        self.handle_openai_responses_stream_response(response, event_tx)
            .await
    }
    ...
    /// Convert Claude-style request into OpenAI Responses format.
    ...
    async fn send_google_request(
        ...
        let base = self.base_url.trim_end_matches('/');
        let api_version = if request.model.to_lowercase().starts_with("gemini-3") {
            "v1alpha"
        } else {
            "v1beta"
        };
        let url = format!(
            ...
            base, api_version, request.model
        );
    ...
    /// Handle OpenAI Responses streaming response.
    async fn handle_openai_responses_stream_response(
        &self,
        response: reqwest::Response,
        event_tx: &mpsc::Sender<AgentEvent>,
    ) -> Result<serde_json::Value, String> {
                ...
                if let Some(data) = line.strip_prefix("data: ") {
                    ...
                    if let Ok(event) = serde_json::from_str::<serde_json::Value>(data) {
                        ...
                        if event["type"].as_str() == Some("response.completed") {
                            if let Some(final_text) = Self::parse_openai_responses_text(&event["response"]) {
                                if !final_text.is_empty() {
                                    accumulated_text = final_text;
                                }
                            }
                            tool_calls = Self::extract_openai_responses_tool_calls(&event["response"]);
                        }
                    }
                }
            }
    ...
    fn extract_openai_responses_tool_calls(response: &serde_json::Value) -> Vec<serde_json::Value> {
    ...
    fn parse_plan_tool_input(&self, input: &serde_json::Value) -> Option<Vec<PlanStepInfo>> {
        ...
        for (idx, step_value) in steps.iter().enumerate() {
            ...
            let step_num = step_value
                .get("step")
                .and_then(|v| v.as_i64())
                .map(|v| v as i32)
                .unwrap_or((idx + 1) as i32);

            plan_steps.push(PlanStepInfo {
                step: step_num,
