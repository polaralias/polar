...
impl LLMClient {
    ...
    pub fn new_with_openai_headers(
        ...
        if let (Some(pid), Some(m)) = (provider_id, model) {
            if pid == "openai" {
                config.api_format = ApiFormat::OpenAIResponses;
            }
        }
    ...
    pub async fn send_message_stream(
        ...
        match self.provider_config.api_format {
            ...
            ApiFormat::Minimax => {
                self.send_openai_compatible(messages, model, max_tokens, temperature, true, Some(tx))
                    .await
            }
        }
    }
    ...
    /// Check if model is a reasoning model family that doesn't support custom temperature
    /// These models only support temperature=1 (default)
    fn is_reasoning_model(model: &str) -> bool {
        let lower = model.to_lowercase();
        // o1/o3/o4, GPT-5 family, codex variants, and computer-use models.
        lower.starts_with("o1")
            || lower.starts_with("o3")
            || lower.starts_with("o4")
    ...
    async fn send_openai_compatible(
        ...
        if self.provider_config.api_format == ApiFormat::OpenAI {
            ...
            if self.provider_config.id == "minimax" {
                // MiniMax OpenAI-compatible docs use max_completion_tokens.
                payload["max_completion_tokens"] = serde_json::json!(max_tokens);
            } else {
                payload["max_tokens"] = serde_json::json!(max_tokens);
            }
    ...
    async fn send_google(
        ...
        // Google Gemini API uses model-dependent versions:
        // Gemini 3 preview: /v1alpha/models/{model}:...
        // Other Gemini models: /v1beta/models/{model}:...
        // or for streaming: :streamGenerateContent?alt=sse
        let base = self.base_url.trim_end_matches('/');
        let api_version = if model.to_lowercase().starts_with("gemini-3") {
            "v1alpha"
        } else {
            "v1beta"
        };
        ...
        let url = if stream {
            format!("{}/{}/models/{}:{}?alt=sse", base, api_version, model, action)
        } else {
            format!("{}/{}/models/{}:{}", base, api_version, model, action)
        };
...
mod tests {
    ...
    fn test_provider_from_model() {
        // Claude model
        let config = ProviderConfig::from_model("claude-sonnet-4-5");
        ...
        // GPT-4o model (OpenAI provider defaults to Responses API)
        ...
        // Computer-use model (uses Responses API)
        let config = ProviderConfig::from_model("computer-use-preview");
        assert_eq!(config.id, "openai");
        assert_eq!(config.api_format, ApiFormat::OpenAIResponses);

        // MiniMax M2 model (OpenAI-compatible API)
        let config = ProviderConfig::from_model("M2");
